function rdt_conformability_video_animation(res_Tg,res_hminv,res_ybr,util_data,res_fyring,res_flring)
...
%the function rdt_conformability_video

res_hminv=res_hminv/1e6;
 Nplot=util_data(1);
 Nr=util_data(2);
 Db=util_data(3);
 gap=util_data(4);
 ...
 v = VideoWriter('C:\Users\YangLiu\Desktop\Aziz Perso\papers mit\RA\RDT\conformability_radial.avi'); %give the user the option where to store the animation file for the total radial force
 vl=VideoWriter('C:\Users\YangLiu\Desktop\Aziz Perso\papers mit\RA\RDT\conformability_liner.avi'); %give the user the option where to store the animation file for the ring liner radial force
 prompt='Frame rate of the animation for the total radial force (number of frames per second) (suggested value: 1): ';
 v.FrameRate=input(prompt);
 prompt='Frame rate of the animation for the ring liner radial force (number of frames per second) (suggested value: 1): ';
 vl.FrameRate=input(prompt);
 open(v);
 open(vl);
...
prompt='Type 1 if you want to play the animations now, 0 otherwise: ';
playnow=input(prompt);

if playnow
    
    prompt='Number of times to play the animation for the total radial force: ';
    numplayradial=input(prompt); 
    prompt='Frame rate of the animation for the total radial force (number of frames per second) (suggested value: 1): ';
    fpsradial=input(prompt); 
    movie(Frame,numplayradial,fpsradial)
    
    prompt='Number of times to play the animation for the ring liner radial force: ';
    numplayliner=input(prompt);
    prompt='Frame rate of the animation for the ring liner radial force (number of frames per second) (suggested value: 1): ';
    fpsliner=input(prompt);
    movie(Frameliner,numplayliner,fpsliner);
