function Fi=Finitial_fs_n4g(Ug,Er,Gr,Iyy,Izz,Jt,alphap,Rr,Telt,Nbe,Npe,Kapfs)
...
for j=1:Nbe
    ...
    indg=(12*(j-1)+1):(12*(j+1));
    
    %nodal y and z displacements for element j
    Uey=Ug([12*(j-1)+1:12*(j-1)+3,12*j+1:12*j+3]);
    Uez=Ug([12*(j-1)+4:12*(j-1)+6,12*j+4:12*j+6]);
    %nodal twist for element j
    Uet=Ug([12*(j-1)+7:12*(j-1)+9,12*j+7:12*j+9]);
...
%     Dgze=sum(dDgze,2);
    ...
    dFite=Le*Er*Izz*repmat((Kapyye-Kapfsyye).*(-(1/Rr-(yr+d2yr)/Rr^2).*sin(alpha+d2zr/Rr)),6,1).*N*dx...
        +Le*Er*Iyy*repmat((Kapzze-Kapfszze).*(1/Rr-(yr+d2yr)/Rr^2).*cos(alpha+d2zr/Rr),6,1).*N*dx...
        +Le*Gr*Jt*repmat(dzr/Rr^2-dalpha/Rr,6,1).*(-dN/Telt/Rr)*dx;
