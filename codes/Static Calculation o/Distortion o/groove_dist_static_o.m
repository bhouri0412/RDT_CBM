function [zgnu,zgnl]=groove_dist_static_o(inputs)
...
%upper flank
for i=1:ngdu
    zgnu(id1)=zgnu(id1)+Agu(i)*sin((i+1)*(Tnod+Phigu(i)))/href;
    zgnu(id2)=zgnu(id2)+(i+1)*Agu(i)*cos((i+1)*(Tnod+Phigu(i)))/href*Telt;
    zgnu(id3)=zgnu(id3)-(i+1)^2*Agu(i)*sin((i+1)*(Tnod+Phigu(i)))/href*Telt^2;
end

%lower flank
for i=1:ngdl
    zgnl(id1)=zgnl(id1)+Agl(i)*sin((i+1)*(Tnod+Phigl(i)))/href;
    zgnl(id2)=zgnl(id2)+(i+1)*Agl(i)*cos((i+1)*(Tnod+Phigl(i)))/href*Telt;
    zgnl(id3)=zgnl(id3)-(i+1)^2*Agl(i)*sin((i+1)*(Tnod+Phigl(i)))/href*Telt^2;
end
end
