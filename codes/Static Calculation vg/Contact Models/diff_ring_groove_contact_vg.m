function [dfdz,dfdalr,dmdz,dmdalr]=...
    diff_ring_groove_contact_vg(gcl,dz_g,zg,zr,alr,beta,beta_th,y1,y2,sigmag,z,Pk,omega,isu)
%alr=alr-alp;
Npe=length(zr);
Nby=50;
...
contact=minh<(omega*sigmag);
%contact region
ncontact=nnz(contact);
if ncontact
    yc=(omega*sigmag-h0(contact))./(-isu*al(contact));
    yc=min(yc,y2);
    yc=max(yc,y1);
